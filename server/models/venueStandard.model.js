...
const venueStandardSchema = new mongoose.Schema(
  {
    venueStandardId: {
      type: Number,
      unique: true
...
venueStandardSchema.pre("save", async function (next) {
  try {
    const counter = await Counter.findOneAndUpdate(
      { name: "venueStandardId" },
      { $inc: { seq: 1 } },
      { new: true, upsert: true }
    );

    this.venueStandardId = counter.seq;
    next();
  } catch (error) {
    next(error);
  }
...
const VenueStandard = mongoose.model("VenueStandard", venueStandardSchema);

module.exports = VenueStandard;
