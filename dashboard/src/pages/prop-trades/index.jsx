...
const ListTradeType = () => {
  ...
  useEffect(() => {
    ...
    if (removeData && removeData?.success) {
      toast.success(removeData?.description, "type-removing");
    }

    if (removeData && !removeData?.success && !isRemoving) {
      toast.error(removeData?.description, { id: "type-removing" });
    }

    if (removeError) {
      toast.error(removeError?.description, { id: "type-removing" });
    }
      ...
      <ControlPanel>
        ...
        {isLoading || (tradeTypes && tradeTypes.length == 0) ? (
          ...
          tradeTypes.map((tradeType) => {
            const {title,description,slug}=tradeType.translations[0].translation.fields
