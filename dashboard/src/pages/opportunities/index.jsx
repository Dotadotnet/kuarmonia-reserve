...
} from "@/services/opportunity/opportunityApi";
import Add from "./add";
...
const ListOpportunity = () => {
  ...
  const { data, isLoading, error, refetch } = useGetOpportunitysQuery({
    page: currentPage,
    limit: itemsPerPage,
  ...
  const opportunitys = useMemo(() => data?.data || [], [data]);

  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال دریافت  ..", { id: "opportunity-loading" });
    }
    if (data) {
      toast.success(data?.description, { id: "opportunity-loading" });
    }


    if (error?.data) {
      toast.error(error?.data?.description, { id: "opportunity-loading" });
    }

    if (isRemoving) {
      toast.loading("در حال حذف  ..", { id: "opportunity-removing" });
    }

    if (removeData && !isRemoving) {
      toast.dismiss("opportunity-removing");
    }

    if (removeError?.data) {
      toast.error(removeError?.data?.message, { id: "opportunity-removing" });
    }
  ...
  return (
    <>
      <ControlPanel>
        ...
        {isLoading || (opportunitys && opportunitys.length == 0) ? (
                ...
                <div className="py-2 flex justify-center items-center gap-x-2 text-right">
                  ...
                  <article className="flex-col flex gap-y-2  ">
                    <span className="line-clamp-1 text-base ">
                      <span className=" ">{opportunity?.title}</span>
                    </span>
              ...
              <div className="lg:col-span-2 hidden gap-2 lg:flex justify-left items-center text-right">
                <article className="flex-col flex gap-y-2">
                  <span className="text-sm lg:text-base overflow-hidden text-ellipsis line-clamp-1">
                    <span className="flex">{opportunity.creator.name}</span>
                  </span>
                </article>
              </div>
              ...
              <div className="col-span-2 md:col-span-1 gap-2 text-center flex justify-center items-center">
                <article className="lg:flex-row flex flex-col justify-center gap-x-2  gap-y-2">
                  ...
                  <DeleteModal
                    ...
                    onDelete={() => removeOpportunity(opportunity?._id)}
                  />
                </article>
              </div>
            </div>
