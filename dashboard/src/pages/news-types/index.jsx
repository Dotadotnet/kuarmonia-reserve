...
const ListNewsType = () => {
  ...
  const { data, isLoading, error, refetch } = useGetNewsTypesQuery();
  const [
    removeNewsType,
  ...
  useEffect(() => {
    ...
    if (isRemoving) {
      toast.loading("در حال حذف  ..", { id: "newsType-removing" });
    }

    if (removeData && !isRemoving) {
      toast.dismiss("newsType-removing");
    }

    if (removeError?.data) {
      toast.error(removeError?.data?.message, { id: "newsType-removing" });
    }
          ...
          categories.map((newsType) => (
            ...
            >
              <div className="col-span-10 lg:col-span-5 text-center flex items-center">
                <StatusIndicator isActive={newsType.status === "active"} />
                <div className="py-2 flex justify-center items-center gap-x-2 text-right">
                  <div className=" flex justify-center items-center">
                    <div
                      dangerouslySetInnerHTML={{ __html: newsType?.icon }}
              ...
              <div className="col-span-2 md:col-span-1 gap-2 text-center flex justify-center items-center">
                <article className="lg:flex-row flex flex-col justify-center gap-x-2  gap-y-2">
                  <Edit id={newsType?._id} />
                  <DeleteModal
                    message="آیا از حذف نوع ملک اطمینان دارید؟"
                    isLoading={isRemoving}
                    onDelete={() => removeNewsType(newsType?._id)}
                  />
                </article>
              </div>
            </div>
