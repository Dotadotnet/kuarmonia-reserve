...
} from "@/services/standard/standardApi";
import Add from "./add";
...
const ListStandard = () => {
  ...
  const { data, isLoading, error, refetch } = useGetStandardsQuery();
  const [
    removeStandard,
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال دریافت  ..", { id: "standard-loading" });
    }
    if (data) {
      toast.success(data?.description, { id: "standard-loading" });
    }

    if (error?.data) {
      toast.error(error?.data?.description, { id: "standard-loading" });
    }

    if (isRemoving) {
      toast.loading("در حال حذف  ..", { id: "standard-removing" });
    }

    if (removeData ) {
      toast.success(removeData?.description,"standard-removing");
    }

    if (removeError?.data) {
      toast.error(removeError?.data?.message, { id: "standard-removing" });
    }
      ...
      <ControlPanel>
        ...
        {isLoading || (standards && standards.length == 0) ? (
          ...
          standards.map((standard) => {
            const {title,description}=standard.translations[0].translation.fields
            ...
            >
              <div className="col-span-10 lg:col-span-5 text-center flex items-center">
                <StatusIndicator isActive={standard.status === "active"} />
                <div className="py-2 flex justify-center items-center gap-x-2 text-right">
                    {standard?.thumbnail ? (
                      <img
                        src={standard?.thumbnail?.url || "/placeholder.png"}
                        height={100}
                        width={100}
  ...
  <img
                        src={standard?.creator?.avatar?.url || "/placeholder.png"}
                        height={100}
                        width={100}
                        className="h-[60px] w-[60px] rounded-full object-cover "
                      />                    )}
              ...
              <div className="lg:col-span-2 hidden gap-2 lg:flex justify-left items-center text-right">
                <article className="flex-col flex gap-y-2">
                  <span className="text-sm lg:text-base overflow-hidden text-ellipsis line-clamp-1">
                    <span className="flex">{standard.creator.name}</span>
                  </span>
                </article>
              </div>
              ...
              <div className="col-span-2 md:col-span-1 gap-2 text-center flex justify-center items-center">
                <article className="lg:flex-row flex flex-col justify-center gap-x-2  gap-y-2">
                  ...
                  <DeleteModal
                    ...
                    onDelete={() => removeStandard(standard?._id)}
                  />
                </article>
              </div>
            </div>
