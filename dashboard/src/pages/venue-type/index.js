...
const ListVenueType = () => {
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال دریافت دسته بندی...", { id: "venueType-loading" });
    }

    if (data && !isLoading) {
      toast.dismiss("venueType-loading");
    }

    if (error?.data) {
      toast.error(error?.data?.message, { id: "venueType-loading" });
    }

    if (isRemoving) {
      toast.loading("در حال دریافت دسته بندی...", { id: "venueType-removing" });
    }
    ...
    if (removeError?.data) {
      toast.error(removeError?.data?.message, { id: "venueType-removing" });
    }
          ...
          categories.map((venueType) => (
            ...
            >
              ...
              <div className="hidden lg:col-span-2 col-span-5 gap-2 text-right lg:flex justify-left items-center">
                <article className="flex-col flex gap-y-2">
                  <span className="flex text-right">{venueType.slug}</span>
                </article>
              </div>
