...
const ListAward = () => {
  ...
  const { data, isLoading, error, refetch } = useGetAwardsQuery({
    page: currentPage,
    limit: itemsPerPage,
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال دریافت  ..", { id: "award-loading" });
    }
    if (data) {
      toast.success(data?.description, { id: "award-loading" });
    }


    if (error?.data) {
      toast.error(error?.data?.description, { id: "award-loading" });
    }

    if (removeData && removeData?.acknowledgement) {
      toast.loading("در حال حذف  ..", { id: "award-removing" });
    }
    if (data) {
      toast.success(data?.description, { id: "award-removing" });
    }


    if (removeError?.data) {
      toast.error(removeError?.data?.description, { id: "award-removing" });
    }
    ...
    <>
      <ControlPanel>
        ...
        {isLoading || (awards && awards.length == 0) ? (
          ...
          awards.map((award) => {
            const {title,description}=award.translations[0].translation.fields
            ...
            >
              <div className="col-span-10 lg:col-span-5 text-center flex items-center">
                <StatusIndicator isActive={award.status === "active"} />
                <div className="py-2 flex justify-center items-center gap-x-2 text-right">
                    {award?.thumbnail ? (
                      <img
                        src={award?.thumbnail?.url || "/placeholder.png"}
                        height={100}
                        width={100}
              ...
              <div className="lg:col-span-2 hidden gap-2 lg:flex justify-left items-center text-right">
                <article className="flex-col flex gap-y-2">
                  <span className="text-sm lg:text-base overflow-hidden text-ellipsis line-clamp-1">
                    <span className="flex">{award.creator.name}</span>
                  </span>
                </article>
              </div>
              ...
              <div className="col-span-2 md:col-span-1 gap-2 text-center flex justify-center items-center">
                <article className="lg:flex-row flex flex-col justify-center gap-x-2  gap-y-2">
                  ...
                  <DeleteModal
                    ...
                    onDelete={() => removeAward(award?._id)}
                  />
                </article>
              </div>
            </div>
