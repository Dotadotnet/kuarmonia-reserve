...
} from "@/services/propStandard/propStandardApi";
import Add from "./add";
...
const ListPropStandard = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 7;
  const [statusFilter, setStatusFilter] = useState("all");
  const [searchTerm, setSearchTerm] = useState("");
  const { data, isLoading, error, refetch } = useGetPropStandardsQuery();
  const [
    removePropStandard,
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال دریافت  ..", { id: "propStandard-loading" });
    }
    if (data) {
      toast.success(data?.description, { id: "propStandard-loading" });
    }

    if (error?.data) {
      toast.error(error?.data?.description, { id: "propStandard-loading" });
    }

    if (isRemoving) {
      toast.loading("در حال حذف  ..", { id: "propStandard-removing" });
    }

    if (removeData ) {
      toast.success(removeData?.description,"propStandard-removing");
    }

    if (removeError?.data) {
      toast.error(removeError?.data?.message, { id: "propStandard-removing" });
    }
          ...
          standards.map((propStandard) => {
            ...
            >
              <div className="col-span-10 lg:col-span-5 text-center flex items-center">
                <StatusIndicator isActive={propStandard.status === "active"} />
                <div className="py-2 flex justify-center items-center gap-x-2 text-right">
                    {propStandard?.thumbnail ? (
                      <img
                        src={propStandard?.thumbnail?.url || "/placeholder.png"}
                        height={100}
                        width={100}
              ...
              <div className="lg:col-span-2 hidden gap-2 lg:flex justify-left items-center text-right">
                <article className="flex-col flex gap-y-2">
                  <span className="text-sm lg:text-base overflow-hidden text-ellipsis line-clamp-1">
                    <span className="flex">{propStandard.creator.name}</span>
                  </span>
                </article>
              </div>
              ...
              <div className="col-span-2 md:col-span-1 gap-2 text-center flex justify-center items-center">
                <article className="lg:flex-row flex flex-col justify-center gap-x-2  gap-y-2">
                  <span
                    ...
                    onClick={() => openEditModal(propStandard)}
                  >
                  ...
                  <DeleteModal
                    ...
                    onDelete={() => removePropStandard(propStandard?._id)}
                  />
                </article>
              </div>
            </div>
...
export default ListPropStandard;
