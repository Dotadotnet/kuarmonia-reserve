...
const StepAddRent = () => {
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال افزودن ...", { id: "add-rent" });
    }

    if (data) {
      toast.success(data?.description, { id: "add-rent" });
      navigate(-1);
    }

    if (error?.data) {
      toast.error(error?.data?.description, { id: "add-rent" });
    }
