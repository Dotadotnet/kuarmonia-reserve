

const Bookmark = ({ className, ...props }) => {
  return (
    <svg
      {...props}
      className={"w-5 h-5" + (className ? " " + className : "")}
      fill="#595959"
      height="87px"
      width="87px"
      version="1.1"
      id="Capa_1"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      viewBox="0 0 55.999 55.999"
      xmlSpace="preserve"
      stroke="#595959"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <path d="M50.25,43.332C49.82,39.225,46.397,36,42.295,36c-2.565,0-4.984,1.286-6.479,3.402c-0.292-0.051-0.588-0.076-0.884-0.076 c-2.32,0-4.381,1.577-5.066,3.813c-1.907,1.131-3.116,3.241-3.116,5.503c0,3.506,2.785,6.358,6.208,6.358h10.774 c0.063,0,0.124-0.004,0.172-0.008C43.965,54.996,44.028,55,44.091,55h4.31c3.225,0,5.849-2.687,5.849-5.989 C54.25,46.411,52.608,44.138,50.25,43.332z M48.401,53l-4.352-0.003l-0.186-0.006L32.958,53c-2.32,0-4.208-1.955-4.208-4.358 c0-1.666,0.948-3.21,2.417-3.935l0.435-0.214l0.102-0.475c0.331-1.56,1.689-2.692,3.229-2.692c0.344,0,0.687,0.057,1.019,0.169 l0.777,0.261l0.409-0.711C38.233,39.139,40.161,38,42.295,38c3.265,0,5.955,2.747,5.999,6.123l0.011,0.813l0.799,0.155 c1.822,0.354,3.146,2.002,3.146,3.919C52.25,51.21,50.523,53,48.401,53z"></path>{" "}
          <path d="M18.75,38.37l-15,15V13h30v21c0,0.553,0.447,1,1,1s1-0.447,1-1V13V0h-34v13v41.443c0,0.631,0.376,1.194,0.959,1.437 c0.193,0.08,0.396,0.119,0.598,0.119c0.403,0,0.8-0.157,1.099-0.456L18.75,41.198l3.401,3.4c0.391,0.391,1.023,0.391,1.414,0 s0.391-1.023,0-1.414L18.75,38.37z M3.75,2h30v9h-30V2z"></path>{" "}
        </g>{" "}
      </g>
    </svg>
  );
};

export default Bookmark;
