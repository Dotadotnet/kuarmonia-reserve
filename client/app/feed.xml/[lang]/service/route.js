...
export async function GET(request) {
    ...
    const items = await Api('/dynamic/get-all/service');

    items.forEach((item) => {
        feed.item({
            title: item.translations[lang.lang].title,
            description: item.translations[lang.lang].summary,
            guid: item.serviceId,
            url: process.env.NEXT_PUBLIC_BASE_URL + ( lang.lang !== "fa" ? "/" +  lang.lang : '' ) + "/service/" + item.serviceId + "/" + encodeURIComponent(item.translations.en.slug),
